// src/utils/profileUtils.ts - Utilitários para perfis de usuário
...
export function createProfilePermissions(
  ...
  const canAccessNotes = (): boolean => {
    // Apenas perfil 3 (gerente do restaurante) pode acessar notas
    return user?.id_perfil === 3;
  };

  const canAccessOrders = (): boolean => {
    return true; // Todos podem acessar pedidos
  };
