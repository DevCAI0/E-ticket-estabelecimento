...
function SummaryCard({ title, value, icon, onClick }: SummaryCardProps) {
  ...
  return (
    <Card
      className="hover:shadow-md transition-shadow flex-1 cursor-pointer"
      onClick={onClick}
    >
      <CardContent className="p-4">
        <div className="flex items-center justify-between space-x-4">
          <div className="flex-1 min-w-0">
            ...
            <div className="text-lg lg:text-xl font-bold">{renderValue()}</div>
          </div>
          <div className="bg-primary/10 p-2 rounded-full flex-shrink-0">{icon}</div>
        </div>
      </CardContent>
    </Card>
  );
